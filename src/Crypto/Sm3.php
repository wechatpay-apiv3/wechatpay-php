...
class Sm3
{
     ...
     *
     * 假设消息`m`的长度为`l`比特。
     * 首先将比特`1`添加到消息的末尾，再添加k个`0`，`k`是满足`l` + `1` + `k` ≡ `448mod512`的最小的非负整数。
     ...
     * 循环计算结果描述为`V(i+1) ← ABCDEF GH ⊕ V(i)`其中，字的存储为大端(`big-endian`)格式。
     *
     * @param int $a
     * @param int $b
     * @param int $c
    ...
    {
        $ss1 = self::rotate((self::rotate($a, 12) + $e + $tj & SM3_UNSIGNED_NO), 7);
        $ss2 = $ss1 ^ self::rotate($a, 12);
