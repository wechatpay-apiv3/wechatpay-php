...
- 对PHP8.2的官方支持，如下PHP8.2的特性需要被提及：
  - ext-openssl 有若干调整，已知在 `OpenSSL3` 上，常量 `RSA_SSLV23_PADDING` 被删除(详细可阅读 openssl/openssl#14216, openssl/openssl#14283)，PHP做了兼容处理，如果扩展依赖的是openssl3，则对应的`OPENSSL_SSLV23_PADDING`常量将不存在，影响到了「非对称加解密混合填充模式的测试用例」覆盖(详情可阅读 shivammathur/setup-php#658)，本类库并不支持此填充模式，顾无影响；删除了依赖`OPENSSL_SSLV23_PADDING`的测试断言，以前向兼容；
