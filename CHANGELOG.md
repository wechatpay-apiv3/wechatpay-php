...
- 修正[DownloadTest.php](./tests/OpenAPI/V3/MerchantService/Images/DownloadTest.php)对请求目标的断言，改为逐一比对预期串：`{var}`简单展开恒为双重编码，`{+var}`保留展开自`guzzlehttp/uri-template@v1.0.6`起保持原串、此前为双重编码，相关修复见[guzzle/uri-template#18](https://github.com/guzzle/uri-template/issues/18)；
